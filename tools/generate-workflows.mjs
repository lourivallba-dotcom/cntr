...
const nResumoProgramacao = add(
  code(
    ...
    [
      ...
      "function bookingBlock(j, i) {",
      "  return ['*' + (i + 1) + ')* \\uD83D\\uDCE6 *' + j.booking + '*', '     \\uD83C\\uDFED ' + (j.loading_plant || '-'), '     \\uD83D\\uDD22 Qtd a montar: ' + (j.qty_containers != null ? j.qty_containers : '?'), ''];",
      "}",
      "const lines = ['\\uD83D\\uDCCB *Programacao atualizada*', items.length + ' booking(s) - ' + newItems.length + ' novo(s), ' + repeatedItems.length + ' repetido(s)/atualizado(s)', ''];",
      "if (newItems.length) {",
      "  lines.push('\\uD83C\\uDD95 *Novos:*');",
      "  newItems.forEach(function(item, i) { lines.push.apply(lines, bookingBlock(item.json, i)); });",
      "}",
      "if (repeatedItems.length) {",
      "  lines.push('\\u26A0\\uFE0F *Ja existiam na base (dados atualizados):*');",
      "  repeatedItems.forEach(function(item, i) { lines.push.apply(lines, bookingBlock(item.json, i)); });",
      "}",
...
const nPrepararInsercao = add(
  code(
    ...
    [
      ...
      "const sql = 'INSERT INTO batch_photos (batch_id, message_id, mime_type, base64_data, photo_type, container_number_text, flex_tank_number_text, flex_lot_no_text, carrier_text) SELECT ' +",
      ...
      "  ' WHERE NOT EXISTS (SELECT 1 FROM batch_photos WHERE batch_id = ' + batch.batch_id + ' AND base64_data = ' + pgQuote(analysis.base64) + ') RETURNING batch_id';",
